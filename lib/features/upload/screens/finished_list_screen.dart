...
class FinishedListScreen extends StatelessWidget {
  const FinishedListScreen({super.key});
    ...
    return BlocBuilder<ListCubit, ListState>(
      builder: (context, state) {
        ...
        return cubit.taskList == null
            ...
            : cubit!.paginationList!.any((task) => task.status == "finished")
            ? ListView.builder(
            padding: const EdgeInsets.all(8.0),
            shrinkWrap: true,
            itemCount: cubit.paginationList?.length,
            itemBuilder: (context, index) {
              return cubit.paginationList![index].status == "finished"
                  ? TaskItem(task: cubit.paginationList![index])
                  : const SizedBox.shrink();
            })
            : const Center(child: Text("No Tasks In Finished List"));
      },
    );
  }
}
